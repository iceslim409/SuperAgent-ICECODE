...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("ICECODE_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("ICECODE_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("ICECODE_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ICECODE_INTERACTIVE=1).
    ...
    try:
        os.environ["ICECODE_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "ICECODE_SPINNER_PAUSE" in os.environ:
            del os.environ["ICECODE_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (ICECODE_INTERACTIVE=1):
    ...
    # ICECODE password prompt or the sudo -S password-pipe path.
    ...
    if not has_configured_password and not sudo_password and os.getenv("ICECODE_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.icecode/.env or environment variables."
        )
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so ICECODE can track the process, then run "
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("ICECODE_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("ICECODE_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("ICECODE_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("ICECODE_SESSION_USER_ID", "")
                        _gw_user_name = _gse("ICECODE_SESSION_USER_NAME", "")
            ...
            try:
                from icecode_cli.hermes_cli.plugins import invoke_hook
