    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "ICECODE Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ICECODE_HOME so each profile gets its own OAuth tokens.
    Layout: ``ICECODE_HOME/mcp-tokens/``
    """
    try:
        from icecode.icecode_constants import get_hermes_home
        base = Path(get_hermes_home())
    except ImportError:
        base = Path(os.environ.get("ICECODE_HOME", str(Path.home() / ".hermes")))
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        ICECODE_HOME/mcp-tokens/<server_name>.json         -- tokens
        ICECODE_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ICECODE_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # ICECODE records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to ICECODE.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "ICECODE Agent")
    scope = cfg.get("scope")
