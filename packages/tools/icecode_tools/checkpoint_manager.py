...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # ICECODE convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("ICECODE_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "ICECODE Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``ICECODE_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ICECODE_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ICECODE_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "ICECODE_WORKDIR"
            if wd_marker.exists():
                try:
