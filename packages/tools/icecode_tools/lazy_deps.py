"""
Lazy dependency installer for opt-in ICECODE Agent backends.
...
(``icecode-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("ICECODE_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
