"""
Cron job management tools for ICECODE Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("ICECODE_SESSION_PLATFORM")
    origin_chat_id = get_session_env("ICECODE_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("ICECODE_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("ICECODE_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within ICECODE_HOME/scripts/.
    ...
    # Only relative paths within ~/.icecode/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.icecode/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.icecode/scripts/ and use just the filename."
        )
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("ICECODE_INTERACTIVE")
        or os.getenv("ICECODE_GATEWAY_SESSION")
        or os.getenv("ICECODE_EXEC_ASK")
    )
