...
# Path to tinker-atropos submodule (relative to icecode-agent root)
ICECODE_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = ICECODE_ROOT / "tinker-atropos"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.icecode/.env",
        }, indent=2)
