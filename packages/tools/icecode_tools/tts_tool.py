...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.icecode/config.yaml``. Hermes
...
Configuration is loaded from ~/.icecode/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    """
    try:
        from icecode_cli.hermes_cli.config import get_env_value as _get_env_value
    except ImportError:
        return os.getenv(name, default)
    value = _get_env_value(name)
    return default if value is None else value
from icecode_tools.managed_tool_gateway import resolve_managed_tool_gateway
from icecode_tools.tool_backend_helpers import managed_nous_tools_enabled, prefers_gateway, resolve_openai_audio_api_key
from icecode_tools.xai_http import hermes_xai_user_agent
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.icecode/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.icecode/config.yaml.
...
# ICECODE without any Python code
...
#           output_format: wav
#
# ICECODE writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ICECODE_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # ICECODE actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.icecode/cache/piper-voices/`` under the active
    ICECODE_HOME so voice downloads follow profile boundaries.
    """
    from icecode.icecode_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.icecode/config.yaml (tts: section).
    ...
    platform = get_session_env("ICECODE_SESSION_PLATFORM", "").lower()
