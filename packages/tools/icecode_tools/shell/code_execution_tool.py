...
"""Auto-generated ICECODE tools RPC stubs."""
...
def _connect():
    ...
    ICECODE_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ICECODE_RPC_SOCKET"]
...
"""Auto-generated ICECODE tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ICECODE_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"ICECODE_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ICECODE_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # selector from ICECODE_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  ICECODE_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ICECODE_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the icecode-agent root is importable in the sandbox so
        ...
        # ICECODE setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ICECODE_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ICECODE_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ICECODE_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.icecode/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (icecode-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and icecode-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.icecode/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call ICECODE tools programmatically. "
