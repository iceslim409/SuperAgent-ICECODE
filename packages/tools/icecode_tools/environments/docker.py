...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.icecode/.env values without failing Docker command execution."""
    try:
        from icecode_cli.hermes_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ICECODE_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ICECODE_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ICECODE_DOCKER_BINARY override: %s", override)
        return override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.icecode/sandboxes/). Non-persistent
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # ICECODE secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ICECODE_PROVIDER_ENV_BLOCKLIST)
