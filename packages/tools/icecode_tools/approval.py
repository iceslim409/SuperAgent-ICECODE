...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ICECODE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ICECODE_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ICECODE_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ICECODE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ICECODE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ICECODE_SESSION_PLATFORM via
    ...
    """
    if os.getenv("ICECODE_CRON_SESSION"):
        return False
    if os.getenv("ICECODE_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ICECODE_HOME.
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ICECODE_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("ICECODE_INTERACTIVE")
    ...
    if is_gateway or os.getenv("ICECODE_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ICECODE_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("ICECODE_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("ICECODE_EXEC_ASK")
