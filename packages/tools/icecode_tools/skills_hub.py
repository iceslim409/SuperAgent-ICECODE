...
class OptionalSkillSource(SkillSource):
    ...
    ~/.icecode/skills/ during setup.  They are discoverable via the Skills Hub
...
# ---------------------------------------------------------------------------
# ICECODE centralized index source
# ---------------------------------------------------------------------------

ICECODE_INDEX_URL = "https://icecode-agent.nousresearch.com/docs/api/skills-index.json"
ICECODE_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "hermes-index.json"
ICECODE_INDEX_TTL = 6 * 3600  # 6 hours
...
def _load_hermes_index() -> Optional[dict]:
    ...
    We cache it locally for ICECODE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ICECODE_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ICECODE_INDEX_CACHE_FILE.stat().st_mtime
            if age < ICECODE_INDEX_TTL:
                return json.loads(ICECODE_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ICECODE_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("ICECODE index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("ICECODE index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ICECODE_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ICECODE_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
