...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ICECODE_HOME", os.path.expanduser("~/.icecode")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ICECODE_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# ICECODE config paths
# ═══════════════════════════════════════════════════════════════════

ICECODE_HOME = Path(os.getenv("ICECODE_HOME", Path.home() / ".hermes"))
CONFIG_PATH = ICECODE_HOME / "config.yaml"
PREFILL_PATH = ICECODE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # ICECODE models — already uncensored, just needs clean prompt
    "hermes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.icecode/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ICECODE for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ICECODE for changes to take effect.")
