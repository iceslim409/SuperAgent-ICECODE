...
try:
    ...
    from icecode.agent.skill_preprocessing import (
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``ICECODE_PLATFORM`` env var and
    ``ICECODE_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("ICECODE_PLATFORM")
            or get_session_env("ICECODE_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("ICECODE_PLATFORM")
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.icecode/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.icecode/skills/`` and any ``skills.external_dirs`` so the
