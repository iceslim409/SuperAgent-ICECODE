...
# All skills live in ~/.icecode/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
ICECODE_HOME = get_hermes_home()
SKILLS_DIR = ICECODE_HOME / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ICECODE_HOME/.env."""
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ICECODE_SESSION_PLATFORM"))
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.icecode/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ICECODE_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ICECODE_PLATFORM`` environment variable
    3. ``ICECODE_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from icecode_cli.hermes_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ICECODE_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.icecode/skills/ and external dirs.
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from icecode_cli.hermes_cli.plugins import _get_disabled_plugins, get_plugin_manager
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.icecode/skills/): {skill_md}")
