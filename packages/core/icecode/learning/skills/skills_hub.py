...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.icecode/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.icecode/skills/<name>/)[/]"
        )
    try:
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with icecode-agent but is not activated by default.\n"
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via ICECODE Skills Hub.\n\n"
                        f"This skill was scanned by the ICECODE Skills Guard before submission.",
