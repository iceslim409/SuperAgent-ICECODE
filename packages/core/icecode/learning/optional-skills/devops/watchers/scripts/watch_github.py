...
Usage (via cron with --no-agent):
    ...
    hermes cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ICECODE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo NousResearch/icecode-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.icecode/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "ICECODE-Watcher/1.0",
    }
