"""Centralized logging setup for ICECODE Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.icecode/logs/`` (profile-aware via ``get_hermes_home()``).
...
from icecode.icecode_constants import get_config_path, get_hermes_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from icecode_cli.hermes_cli.config import is_managed
