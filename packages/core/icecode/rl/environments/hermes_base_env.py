...
Provides the Atropos integration plumbing that all icecode-agent environments share:
...
# Ensure the icecode-agent repo root is on sys.path so that imports like
...
# Load API keys from icecode-agent/.env so all environments can access them
...
# Import icecode-agent toolset infrastructure
...
class HermesAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for icecode-agent Atropos environments.
...
class HermesAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for icecode-agent Atropos integration.
    ...
    name: Optional[str] = "icecode-agent"
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY icecode-agent tool (terminal, file, web,
