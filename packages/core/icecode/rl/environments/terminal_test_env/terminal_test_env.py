...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from ICECODE Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from ICECODE Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from ICECODE Agent",
    },
    {
...
class TerminalTestEnv(HermesAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/icecode-agent/.env.
        """
