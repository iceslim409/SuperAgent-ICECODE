...
Runs the icecode-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to icecode-agent's run_agent.py. Tool execution is dispatched via
...
class HermesAgentLoop:
    """
    Runs icecode-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # icecode-agent's standalone parsers. This handles the case where
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via icecode-agent's dispatch
