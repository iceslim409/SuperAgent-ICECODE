...
Resolution order:
  1. ``ICECODE_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.icecode/config.yaml``
...
from icecode.icecode_constants import get_config_path
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("ICECODE_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
