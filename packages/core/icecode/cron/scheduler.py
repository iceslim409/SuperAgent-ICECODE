...
Uses a file-based lock (~/.icecode/cron/.tick.lock) so only one tick
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    if per_job:
        return per_job
    try:
        from icecode_cli.hermes_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_hermes_home() -> Path:
    """Resolve ICECODE home dynamically while preserving test monkeypatch hooks."""
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if ":" in deliver_value:
        ...
        from icecode_tools.send_message_tool import _parse_target_ref
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    from icecode_tools.send_message_tool import _send_to_platform
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ICECODE_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ICECODE_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ICECODE_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ICECODE_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within ICECODE_HOME/scripts/.
    try:
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    from icecode_tools.skills_tool import skill_view
...
def _scan_assembled_cron_prompt(assembled: str, job: dict) -> str:
    ...
    """
    from icecode_tools.cronjob_tools import _scan_cron_prompt
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["ICECODE_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed ICECODE_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ICECODE_SESSION_PLATFORM
    #     and ICECODE_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ICECODE_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ICECODE_SESSION_PLATFORM == "telegram".
    ...
    #     ICECODE_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ICECODE_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ICECODE_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ICECODE_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "ICECODE_CRON_AUTO_DELIVER_PLATFORM",
        "ICECODE_CRON_AUTO_DELIVER_CHAT_ID",
        "ICECODE_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        if delivery_target:
            _VAR_MAP["ICECODE_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ICECODE_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ICECODE_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("ICECODE_MODEL") or ""
        ...
        prefill_file = os.getenv("ICECODE_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
        ...
        try:
            # Do not inject ICECODE_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        try:
            from icecode_tools.mcp_tool import discover_mcp_tools
        ...
        agent = AIAgent(
            ...
            # ICECODE_HOME. When a workdir is configured, also inject project
        ...
        # override via ICECODE_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ICECODE_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ICECODE_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # Set ICECODE_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ICECODE_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ICECODE_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        try:
            from icecode_tools.mcp_tool import _kill_orphaned_mcp_children
