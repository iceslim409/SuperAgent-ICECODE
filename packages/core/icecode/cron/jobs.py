...
Jobs are stored in ~/.icecode/cron/jobs.json
Output is saved to ~/.icecode/cron/output/{job_id}/{timestamp}.md
"""
...
ICECODE_DIR = get_hermes_home().resolve()
CRON_DIR = ICECODE_DIR / "cron"
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall icecode-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.icecode/scripts/; ``.sh`` / ``.bash`` files run via bash,
