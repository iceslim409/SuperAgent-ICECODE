...
Sessions are persisted to the shared SessionDB (``~/.icecode/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    """
    from icecode.icecode_constants import is_wsl
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed ICECODE agent."""
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            ...
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.icecode/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ICECODE_HOME`` dynamically rather than relying on
