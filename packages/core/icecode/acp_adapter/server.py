"""ACP agent server — exposes ICECODE Agent via the Agent Client Protocol."""
...
try:
    from icecode_cli.hermes_cli import __version__ as ICECODE_VERSION
except Exception:
    ICECODE_VERSION = "0.0.0"
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; ICECODE cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a ICECODE/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping ICECODE AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show ICECODE version",
        },
    )
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from icecode_cli.hermes_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from icecode_cli.hermes_cli.models import detect_provider_for_model, parse_model_input
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate ICECODE using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="icecode-agent", version=ICECODE_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # ICECODE' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run ICECODE on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ICECODE' local kawaii waiting/status
        ...
        # not the executor's. Also set ICECODE_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ICECODE_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ICECODE_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ICECODE_INTERACTIVE")
            ...
            try:
                ...
                # Restore ICECODE_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ICECODE_INTERACTIVE", None)
                else:
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (ICECODE_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"ICECODE Agent v{ICECODE_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when ICECODE has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
