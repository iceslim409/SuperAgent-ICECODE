"""CLI entry point for the icecode-agent ACP adapter.

Loads environment variables from ``~/.icecode/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ICECODE_HOME (default ``~/.icecode``)."""
...
def main() -> None:
    ...
    logger.info("Starting icecode-agent ACP adapter")
