"""engine_conversation_mixin — main conversation loop.

Extracted from engine.py: run_conversation(), chat(), and
_run_codex_app_server_turn(). These are the top-level entry points that
...
try:
    ...
    from icecode.agent.error_classifier import FailoverReason
except (ImportError, AttributeError):
    def get_hermes_home():
        from pathlib import Path
        return Path.home() / ".icecode"
    def _sanitize_surrogates(*a, **kw): return a[0] if a else ""
    def _sanitize_structure_surrogates(*a, **kw): return a[0] if a else {}
    def _sanitize_messages_surrogates(*a, **kw): return a[0] if a else []
...
class _ConversationMixin:
    ...
    def run_conversation(
        ...
        try:
            from icecode.agent.auxiliary_client import set_runtime_main
            set_runtime_main(
        ...
        from icecode.icecode_logging import set_session_context
        ...
        # ICECODE path is bypassed entirely.
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # ICECODE internals.
            #
            # ICECODE invariant: the system prompt is built ONCE per session
            ...
            while retry_count < max_retries:
                ...
                if self.provider == "nous":
                    try:
                        from icecode.agent.nous_rate_guard import (
                ...
                try:
                    ...
                    if env_var_enabled("ICECODE_DUMP_REQUESTS"):
                    ...
                    if self.provider == "nous":
                        try:
                            from icecode.agent.nous_rate_guard import clear_nous_rate_limit
                    ...
                    if (
                        ...
                        from icecode.icecode_constants import display_hermes_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for ICECODE-managed OAuth/setup tokens")
        ...
        if final_response is None and (
            ...
            _kanban_task = os.environ.get("ICECODE_KANBAN_TASK")
            if _kanban_task:
                try:
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        """
        from icecode.agent.transports.codex_app_server_session import CodexAppServerSession
        ...
        if not hasattr(self, "_codex_session") or self._codex_session is None:
            ...
            # ICECODE' standard prompt flow if a
