...
Nous can attribute usage to ICECODE Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=icecode-agent",
        "client=hermes-client-v<__version__>",
    ]

The version is sourced live from ``icecode.__version__`` so it auto-aligns
...
``icecode.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current ICECODE release version, e.g. ``"0.13.0"``.
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=icecode-agent", hermes_client_tag()]
