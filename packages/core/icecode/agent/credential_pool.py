...
from icecode_cli.hermes_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_credentials_file(self, entry: PooledCredential) -> PooledCredential:
        ...
        if self.provider != "anthropic" or entry.source != "claude_code":
            return entry
        try:
            from icecode.agent.anthropic_adapter import read_claude_code_credentials
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from icecode.agent.anthropic_adapter import refresh_anthropic_oauth_pure
                ...
                if entry.source == "claude_code":
                    try:
                        from icecode.agent.anthropic_adapter import _write_claude_code_credentials
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from icecode_cli.hermes_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        from icecode.agent.anthropic_adapter import read_claude_code_credentials, read_hermes_oauth_credentials
        ...
        # ICECODE auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from icecode_cli.hermes_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.icecode/auth.json providers.minimax-oauth.
        ...
        # ICECODE auth store.  Without this gate the removal is instantly
        ...
        # ICECODE owns its own Codex auth state — we do NOT auto-import from
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.icecode/.env over os.environ — the user's config file is the
    # ICECODE credentials. Stale env vars from parent
    ...
    # won't be re-seeded from the user's shell environment or ~/.icecode/.env.
    ...
    try:
        from icecode_cli.hermes_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.icecode/.env over os.environ
