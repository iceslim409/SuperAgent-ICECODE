...
# ICECODE' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ICECODE_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ICECODE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ICECODE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # ICECODE' text path is the common case).
        try:
...
def _get_hermes_version() -> str:
    """Best-effort ICECODE version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("icecode-agent")
