...
# ICECODE (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. ICECODE' default runtime
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on ICECODE_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.icecode/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on ICECODE_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    ...
    # ICECODE tools so dispatch works.
    ...
    mcp = FastMCP(
        "hermes-tools",
        instructions=(
            "ICECODE Agent's tool surface, exposed for use inside a Codex "
    ...
    # ICECODE tool schemas for the ones we expose, so
    # ICECODE gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this ICECODE process", name
            )
            continue

        description = spec.get("description") or f"ICECODE {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # ICECODE' own banners off stdout (which is the MCP wire).
    os.environ.setdefault("ICECODE_QUIET", "1")
    os.environ.setdefault("ICECODE_REDACT_SECRETS", "true")
