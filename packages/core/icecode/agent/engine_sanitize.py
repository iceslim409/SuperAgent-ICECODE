"""engine_sanitize — message/string sanitization utilities for the AI agent.

Extracted from engine.py to keep pure-function helpers testable in isolation.
All functions are stateless and depend only on stdlib (re, json).
"""
...
def _sanitize_messages_surrogates(messages: list) -> bool:
    ...
    Covers content/text, name, tool call metadata/arguments, and any additional
    string or nested structured fields (reasoning, reasoning_content, etc.).
    """
    found = False
    for msg in messages:
...
def _repair_tool_call_arguments(raw_args: str, tool_name: str = "?") -> str:
    ...
    Python ``None``, etc. Returns ``"{}"`` as last resort so the request succeeds.
    """
    ...
    # Repair pass 0: strict=False handles literal control chars in strings
    try:
...
def _strip_non_ascii(text: str) -> str:
    """Remove non-ASCII characters; last resort for ASCII-only environments."""
...
def _sanitize_messages_non_ascii(messages: list) -> bool:
    ...
    Last-resort recovery for systems with ASCII-only encoding (LANG=C,
    Chromebooks, minimal containers). Returns True if any content was sanitized.
    """
    found = False
    for msg in messages:
        if not isinstance(msg, dict):
...
def _strip_images_from_messages(messages: list) -> bool:
    ...
    Called when a server signals it does not support images. Preserves
    tool_call_id linkage by replacing image-only tool responses with a
    placeholder rather than deleting them. Returns True if any parts removed.
    """
    found = False
    to_delete = []
