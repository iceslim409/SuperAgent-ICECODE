...
``<ICECODE_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
# ``<ICECODE_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def hermes_lsp_bin_dir() -> Path:
    """Return the ICECODE-owned bin staging dir for LSP servers."""
    home = os.environ.get("ICECODE_HOME")
    if home is None:
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <ICECODE_HOME>/lsp/
