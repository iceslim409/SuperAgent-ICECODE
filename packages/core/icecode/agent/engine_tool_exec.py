"""engine_tool_exec — tool execution mixin for AIAgent.

Extracted from engine.py: _execute_tool_calls, _dispatch_delegate_task,
_invoke_tool, _wrap_verbose, _execute_tool_calls_concurrent,
_execute_tool_calls_sequential. All methods are pure AIAgent logic; they
use ``self`` freely and are composed into the class via mixin inheritance.
"""
...
# Pure extracted modules — no heavy runtime deps, safe to import at module level.
from icecode.agent.engine_tool_guard import _is_destructive_command, _should_parallelize_tool_batch
...
# Display/tools imports — real in production; lightweight stubs in test env where
# agent.display resolves to the packages/tools stub without KawaiiSpinner.
try:
    ...
    def _build_tool_preview(*a, **kw): return ""  # type: ignore[no-redef]
    def _get_cute_tool_message_impl(*a, **kw): return ""  # type: ignore[no-redef]
    def _detect_tool_failure(*a, **kw): return (False, "")  # type: ignore[no-redef]
    def _get_tool_emoji(*a, **kw): return "⚡"  # type: ignore[no-redef]
    def handle_function_call(*a, **kw): return "{}"  # type: ignore[no-redef]
    def get_active_env(*a, **kw): return None  # type: ignore[no-redef]
    def _set_approval_callback(*a, **kw): pass  # type: ignore[no-redef]
    def _set_sudo_password_callback(*a, **kw): pass  # type: ignore[no-redef]
    def _get_approval_callback(*a, **kw): return None  # type: ignore[no-redef]
    def _get_sudo_password_callback(*a, **kw): return None  # type: ignore[no-redef]
    def maybe_persist_tool_result(*a, **kw): return a[0] if a else ""  # type: ignore[no-redef]
    def enforce_turn_budget(*a, **kw): pass  # type: ignore[no-redef]
    def _set_interrupt(*a, **kw): pass  # type: ignore[no-redef]


class _ToolExecutionMixin:
    """Mixin providing tool execution methods for AIAgent.

    All methods reference ``self`` attributes defined by AIAgent.__init__;
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            try:
                from icecode_cli.hermes_cli.plugins import get_pre_tool_call_block_message
