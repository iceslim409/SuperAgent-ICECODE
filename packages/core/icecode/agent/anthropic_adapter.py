"""Anthropic Messages API adapter for ICECODE Agent.
...
try:
    ...
    from icecode.utils import base_url_host_matches, normalize_proxy_env_vars
...
# ICECODE effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    # ICECODE-managed OAuth/setup token env var
...
# ICECODE-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.icecode/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ICECODE_OAUTH_FILE = get_hermes_home() / ".anthropic_oauth.json"
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run ICECODE-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize ICECODE with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read ICECODE-managed OAuth credentials from ~/.icecode/.anthropic_oauth.json."""
    if _ICECODE_OAUTH_FILE.exists():
        try:
            data = json.loads(_ICECODE_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read ICECODE OAuth credentials: %s", e)
    return None
...
def _normalize_tool_input_schema(schema: Any) -> Dict[str, Any]:
    ...
    from icecode_tools.schema_sanitizer import strip_nullable_unions
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See icecode-agent#13848.
            #
    ...
    # are preserved.  See icecode-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("ICECODE Agent", "Claude Code")
                text = text.replace("ICECODE agent", "Claude Code")
                text = text.replace("icecode-agent", "claude-code")
    ...
    # ICECODE surfaces in its CLI. We
