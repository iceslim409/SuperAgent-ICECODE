...
_ICECODE_MD_NAMES = (".hermes.md", "HERMES.md")
...
DEFAULT_AGENT_IDENTITY = (
    "You are ICECODE Agent, an intelligent AI assistant created by Nous Research. "
...
ICECODE_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using ICECODE Agent "
    "itself, load the `icecode-agent` skill with skill_view(name='icecode-agent') "
    "before answering. Docs: https://icecode-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.icecode/kanban.db`. Your task id is in "
    "`$ICECODE_KANBAN_TASK`; your workspace is `$ICECODE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ICECODE_KANBAN_WORKSPACE` before "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the ICECODE WebUI, a browser-based chat interface. "
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where ICECODE itself is running. The host OS, home, and cwd "
                f"of the ICECODE process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where ICECODE "
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.icecode/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ICECODE_PLATFORM")
        or get_session_env("ICECODE_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot ICECODE Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `icecode-agent` skill "
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ICECODE_HOME and return its content, or None.
    ...
    """
    try:
        from icecode_cli.hermes_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure ICECODE_HOME before loading SOUL.md: %s", e)
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from ICECODE_HOME is independent and always included when present.
    ...
    # SOUL.md from ICECODE_HOME only — skip when already loaded as identity
    if not skip_soul:
