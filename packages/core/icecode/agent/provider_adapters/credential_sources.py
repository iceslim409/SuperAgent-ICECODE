...
Hermes seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.icecode/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.icecode/.anthropic_oauth.json
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.icecode/.env  → clear it
    ...
    """
    from icecode_cli.hermes_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.icecode/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to ICECODE.",
            f"  The pool entry is now suppressed — ICECODE will ignore "
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.icecode/.anthropic_oauth.json is ours — delete it outright."""
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared ICECODE Anthropic OAuth credentials")
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    """
    from icecode_cli.hermes_cli.auth import suppress_credential_source
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.icecode/.anthropic_oauth.json",
    ))
    register(RemovalStep(
