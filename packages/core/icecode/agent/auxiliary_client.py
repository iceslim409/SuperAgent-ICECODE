...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.icecode/auth.json active provider)
...
from icecode.agent.credential_pool import load_pool
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://icecode-agent.nousresearch.com",
    "X-Title": "ICECODE Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ICECODE_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ICECODE_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ICECODE_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ICECODE_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
#
# The tags are computed from icecode.agent.portal_tags so the client= marker stays
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``icecode.__version__`` is
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (ICECODE Agent)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from icecode.agent.anthropic_adapter import build_anthropic_kwargs
        ...
        if temperature is not None:
            from icecode.agent.anthropic_adapter import _forbids_sampling_params
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from icecode.agent.gemini_native_adapter import GeminiNativeClient
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.icecode/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from icecode_cli.hermes_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from icecode_cli.hermes_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from icecode_cli.hermes_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from icecode.agent.nous_rate_guard import nous_rate_limit_remaining
    ...
    model = _NOUS_MODEL
    try:
        from icecode_cli.hermes_cli.models import get_nous_recommended_aux_model
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from icecode.agent.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    old OPENAI_BASE_URL lingers in ~/.icecode/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from ~/.icecode/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from icecode.agent.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        try:
            from icecode.agent.model_metadata import _infer_provider_from_url
...
def resolve_provider_client(
    ...
    try:
        from icecode_cli.hermes_cli.runtime_provider import _get_named_custom_provider
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from icecode.agent.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
