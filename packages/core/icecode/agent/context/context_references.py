...
from icecode.agent.model_metadata import estimate_tokens_rough
...
_SENSITIVE_ICECODE_DIRS = (Path("skills") / ".hub",)
...
async def _default_url_fetcher(url: str) -> str:
    from icecode_tools.web_tools import web_extract_tool
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_dirs.extend(hermes_home / rel for rel in _SENSITIVE_ICECODE_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal ICECODE path and cannot be attached")
