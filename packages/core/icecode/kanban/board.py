...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ICECODE_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ICECODE_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
In standard installs ``<root>`` is ``~/.icecode``. In Docker / custom
deployments where ``ICECODE_HOME`` points outside ``~/.icecode`` (e.g.
``/opt/hermes``), ``<root>`` is ``ICECODE_HOME``. Legacy env-var
overrides still work:

* ``ICECODE_KANBAN_DB`` — pin the database file path directly.
* ``ICECODE_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ICECODE_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ICECODE_KANBAN_DB``,
``ICECODE_KANBAN_WORKSPACES_ROOT``, and ``ICECODE_KANBAN_BOARD`` into
...
# enough that kebab-case names like ``atm10-server`` or ``icecode-agent``
...
def kanban_home() -> Path:
    ...
    1. ``ICECODE_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``ICECODE_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ICECODE_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ICECODE_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ICECODE_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    ...
    1. ``ICECODE_KANBAN_BOARD`` env var (set by the dispatcher on worker
    ...
    """
    env = os.environ.get("ICECODE_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ICECODE_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ICECODE_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ICECODE_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ICECODE_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ICECODE_KANBAN_DB`` env → ``ICECODE_KANBAN_BOARD`` env →
    ...
    from icecode.icecode_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from icecode_cli.hermes_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from icecode_cli.hermes_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _default_spawn(
    ...
    *,
    ...
    ``ICECODE_KANBAN_DB`` / ``ICECODE_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ICECODE_HOME so the worker reads the profile-scoped config.yaml
    ...
    # If ICECODE_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".hermes" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from icecode_cli.hermes_cli.profiles import resolve_profile_env
    try:
        env["ICECODE_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ICECODE_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ICECODE_HOME never had profiles created.
        pass
    if task.tenant:
        env["ICECODE_TENANT"] = task.tenant
    env["ICECODE_KANBAN_TASK"] = task.id
    env["ICECODE_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["ICECODE_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ICECODE_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`hermes -p <name>`
    # rewrites ICECODE_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ICECODE_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ICECODE_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ICECODE_KANBAN_BOARD"] = resolved_board
    # ICECODE_PROFILE is the author the kanban_comment tool defaults to.
    # `hermes -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ICECODE_PROFILE"] = profile_arg
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install ICECODE Agent or activate its venv before running the kanban dispatcher."
        )
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ICECODE_PROFILE values like "hermes-system"
