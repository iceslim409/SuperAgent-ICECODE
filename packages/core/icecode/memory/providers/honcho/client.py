...
Resolution order for config file:
  1. $ICECODE_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
from icecode_cli.hermes_cli.profiles import _get_default_hermes_home
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. ICECODE_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``hermes.<profile>``
      3. Fallback: ``"hermes"`` (default profile)
    """
    explicit = os.environ.get("ICECODE_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ICECODE_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.icecode/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        Resolution: $ICECODE_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
    ...
    def resolve_session_name(
        ...
        # ICECODE session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    try:
        from icecode_tools.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
