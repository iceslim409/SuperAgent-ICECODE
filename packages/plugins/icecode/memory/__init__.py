...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with icecode-agent)
2. User-installed providers: ``$ICECODE_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ICECODE_HOME/plugins/`` or None if unavailable."""
    try:
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ICECODE_HOME/plugins/<name>/``) directories.  Bundled takes
