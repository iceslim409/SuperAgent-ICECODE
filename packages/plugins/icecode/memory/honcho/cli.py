...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current ICECODE profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ICECODE_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives ICECODE persistent cross-session memory.")
...
def _active_profile_name() -> str:
    """Return the active ICECODE profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    ICECODE' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. ICECODE asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → ICECODE + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → ICECODE\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives ICECODE persistent memory")
    ...
    if user_files:
        ...
        print("  (ICECODE calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In ICECODE, they are seeded once into Honcho's AI peer through the")
    ...
    print("    ICECODE:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    ICECODE:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    ICECODE:   Honcho observes every message and updates representations")
    ...
    print("    ICECODE:   per-session by default — each run gets its own session")
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from icecode_cli.hermes_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to ICECODE Honcho",
    )
