...
session-scoped state file under ``$ICECODE_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ICECODE_HOME):
#
#   workspace/meetings/
...
def start(
    ...
    *,
    ...
    env = os.environ.copy()
    env["ICECODE_MEET_URL"] = url
    env["ICECODE_MEET_OUT_DIR"] = str(out)
    env["ICECODE_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ICECODE_MEET_HEADED"] = "1"
    if auth_state:
        env["ICECODE_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ICECODE_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ICECODE_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ICECODE_MEET_MODE"] = mode
    if realtime_model:
        env["ICECODE_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ICECODE_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ICECODE_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ICECODE_MEET_REALTIME_KEY"] = realtime_api_key
