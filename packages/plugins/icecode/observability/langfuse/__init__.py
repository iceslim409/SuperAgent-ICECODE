...
Required env vars (set via ``hermes tools`` or ~/.icecode/.env):
  ICECODE_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ICECODE_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ICECODE_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ICECODE_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ICECODE_LANGFUSE_RELEASE     - release/version tag
  ICECODE_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  ICECODE_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ICECODE_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("ICECODE_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("ICECODE_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("ICECODE_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    if not (public_key and secret_key):
        _LANGFUSE_CLIENT = _INIT_FAILED
        return None

    base_url = _env("ICECODE_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("ICECODE_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("ICECODE_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("ICECODE_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid ICECODE_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("ICECODE_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # ICECODE branches used pre_llm_call for request-scoped tracing and
    # ICECODE also has a turn-scoped
...
def register(ctx) -> None:
    ...
    # ICECODE versions.  pre_api_request / post_api_request fire per API
