...
#     fresh "ICECODE is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ICECODE_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "ICECODE is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ICECODE_HOME", str(_Path.home() / ".hermes"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ICECODE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'ICECODE is thinking…' marker message.
        ...
        "ICECODE is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "ICECODE is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "ICECODE is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "ICECODE is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "ICECODE is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    ...
    prompt for env vars, persist them to ``~/.icecode/.env`` so the next
    gateway restart picks them up.
    """
    from icecode_cli.hermes_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print()
    print_success("Google Chat configuration saved to ~/.icecode/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'icecode-agent[google_chat]'",
        ...
        # the "ICECODE is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'ICECODE is thinking…' marker message "
