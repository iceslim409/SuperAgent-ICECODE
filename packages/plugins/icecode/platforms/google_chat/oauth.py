...
- Per-user tokens (keyed by sender email):
    ``${ICECODE_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ICECODE_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ICECODE_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ICECODE_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${ICECODE_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ICECODE_HOME helper so the token follows the user's
# profile (e.g. tests can override via ICECODE_HOME=/tmp/...).
try:
    from icecode.icecode_constants import display_hermes_home, get_hermes_home
...
def _hermes_home() -> Path:
    """Resolve ICECODE_HOME at call time (NOT module import).

    Tests and ``ICECODE_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.icecode/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install icecode-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'icecode-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ICECODE_HOME."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for ICECODE (native attachment delivery)"
    )
