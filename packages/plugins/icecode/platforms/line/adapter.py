"""
LINE Messaging API platform adapter for ICECODE Agent.
...
to ICECODE Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ICECODE_HOME``. PR #8398.
        """
        from aiohttp import web
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.icecode/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        from icecode_cli.hermes_cli.config import get_env_var, set_env_var
    except ImportError:
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.icecode/.env")
        return
...
def register(ctx) -> None:
    """Plugin entry point — called by the ICECODE plugin system at startup."""
