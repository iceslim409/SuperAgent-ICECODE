"""
IRC Platform Adapter for ICECODE Agent.
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :ICECODE Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :ICECODE Agent shutting down")
...
def interactive_setup() -> None:
    ...
    print_info("Connect ICECODE to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    print()
    print_success("IRC configuration saved to ~/.icecode/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :ICECODE Agent (cron)")
...
def register(ctx):
    """Plugin entry point: called by the ICECODE plugin system."""
