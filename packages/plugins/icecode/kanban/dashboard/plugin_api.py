...
own pages inject it via ``window.__ICECODE_SESSION_TOKEN__`` so logged-in
...
from icecode_cli.hermes_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from icecode_cli.hermes_cli import web_server as _ws
    except Exception:
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from icecode_cli.hermes_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``ICECODE_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from icecode_cli.hermes_cli.kanban import _check_dispatcher_presence
...
def specify_task_endpoint(
    ...
    prev_env = os.environ.get("ICECODE_KANBAN_BOARD")
    try:
        os.environ["ICECODE_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from icecode_cli.hermes_cli import kanban_specify  # noqa: WPS433 (intentional)
        ...
        if prev_env is None:
            ...
            os.environ["ICECODE_KANBAN_BOARD"] = prev_env
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.icecode/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.icecode/profiles/*`` on disk and every
