"""disk_cleanup — ephemeral file cleanup for ICECODE Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ICECODE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ICECODE_HOME and /tmp/hermes-*
Never touches: ~/.icecode/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ICECODE_HOME") or "").strip()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ICECODE_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ICECODE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ICECODE_HOME or ``/tmp/hermes-*``.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ICECODE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ICECODE_HOME (but leave ICECODE_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "icecode-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under ICECODE_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
