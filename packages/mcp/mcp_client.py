...
transport, discovers their tools, and registers them into the icecode-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.icecode/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.icecode/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "ICECODE_HOME", os.path.join(os.path.expanduser("~"), ".hermes")
                )
            )
            candidates = [
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.icecode/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.icecode/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # ICECODE' MEDIA: tag convention and out to messaging
            ...
            # ICECODE' MEDIA tag + cache_image_from_bytes) was the cleaner of
