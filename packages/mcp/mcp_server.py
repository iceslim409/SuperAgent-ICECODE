...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ICECODE_HOME."""
    try:
        from icecode.icecode_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ICECODE_HOME", Path.home() / ".hermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("ICECODE_HOME", Path.home() / ".hermes")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("ICECODE_HOME", Path.home() / ".hermes")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the ICECODE MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "ICECODE Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ICECODE MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
