#!/usr/bin/env python3
"""
ICECODE Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the ICECODE Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills icecode-agent-dev,github-auth
...
os.environ["ICECODE_QUIET"] = "1"  # Our own modules
...
try:
    from icecode_cli.hermes_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from icecode_cli.hermes_cli.banner import _format_context_length, format_banner_version_label
...
from icecode_cli.hermes_cli.browser_connect import (
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.icecode/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ICECODE_IGNORE_USER_CONFIG=1 is set (via ``hermes chat --ignore-user-config``),
    the user config at ``~/.icecode/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ICECODE_HOME}/config.yaml)
    user_config_path = _hermes_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ICECODE_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain ICECODE, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me ICECODE - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
    ...
    from icecode_cli.hermes_cli.config import _expand_env_vars
    ...
    # UNLESS we're inside a gateway process (detected by _ICECODE_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_ICECODE_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ICECODE_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.icecode/logs/.
...
try:
    from icecode_cli.hermes_cli.config import print_config_warnings
...
try:
    from icecode_cli.hermes_cli.skin_engine import init_skin_from_config
...
from icecode_cli.hermes_cli.banner import build_welcome_banner
from icecode_cli.hermes_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from icecode_cli.hermes_cli.callbacks import prompt_for_secret
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from icecode_cli.hermes_cli.skin_engine import get_active_skin
...
from icecode.icecode_constants import is_termux as _is_termux_environment
...
ICECODE_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ICECODE Caduceus (compact, fits in left panel)
ICECODE_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "ICECODE Agent") if _skin else "ICECODE Agent"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from icecode_cli.hermes_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.icecode/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.icecode/config.yaml on first use)
...
class HermesCLI:
    """
    Interactive CLI for the ICECODE Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from icecode_cli.hermes_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("ICECODE_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ICECODE_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ICECODE_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        self.ignore_rules = ignore_rules or os.environ.get("ICECODE_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ICECODE_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # ICECODE processes for this ICECODE_HOME.
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from icecode_cli.hermes_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'ICECODE'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from icecode_cli.hermes_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from icecode_cli.hermes_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from icecode_cli.hermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ ICECODE")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from icecode_cli.hermes_cli.runtime_provider import (
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   ICECODE needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # ICECODE LLM (not agentic)
        from icecode_cli.hermes_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research ICECODE 3 & 4 models are NOT agentic and are not "
                "designed for use with ICECODE Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ ICECODE: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ ICECODE: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.icecode/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from icecode_cli.hermes_cli.clipboard import save_clipboard_image
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        """
        from icecode_cli.hermes_cli.backup import (
    ...
    def _handle_paste_command(self):
        ...
        from icecode_cli.hermes_cli.clipboard import has_clipboard_image
    ...
    def show_help(self):
        ...
        from icecode_cli.hermes_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from icecode_cli.hermes_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with ICECODE!{_RST}")
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from icecode_cli.hermes_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ICECODE_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        # Resolve title or ID
        from icecode_cli.hermes_cli.main import _resolve_session_by_name_or_id
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.icecode/sessions/saved/.
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from icecode_cli.hermes_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from icecode_cli.hermes_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from icecode_cli.hermes_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from icecode_cli.hermes_cli.model_switch import switch_model, parse_model_flags
        ...
        # clobber disk config.
        from icecode_cli.hermes_cli.inventory import build_models_payload, load_picker_context
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from icecode_cli.hermes_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        try:
            from icecode_cli.hermes_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        from icecode_cli.hermes_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        from icecode_cli.hermes_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from icecode_cli.hermes_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from icecode_cli.hermes_cli.plugins import get_plugin_manager
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from icecode_cli.hermes_cli.commands import COMMANDS
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"ICECODE keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from icecode_cli.hermes_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from icecode_cli.hermes_cli.config import load_config
        from icecode_cli.hermes_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("ICECODE_YOLO_MODE"))
        if current:
            os.environ.pop("ICECODE_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while ICECODE is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while ICECODE is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from icecode_cli.hermes_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from icecode_cli.hermes_cli.debug import run_debug_share
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from icecode_cli.hermes_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.icecode/skills/ and queue a note for the
        next user turn.
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from icecode_cli.hermes_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from icecode_cli.hermes_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from icecode_cli.hermes_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to ICECODE Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to ICECODE Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(ICECODE_REDACT_SECRETS={_redact_raw}). "
        ...
        # ICECODE migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from icecode_cli.hermes_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from icecode_cli.hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "ICECODE Agent")
        ...
        try:
            from icecode_cli.hermes_cli.config import load_config
            from icecode_cli.hermes_cli.voice import (
        ...
        # ICECODE symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("ICECODE needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'ICECODE needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("ICECODE needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ICECODE_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ICECODE_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
...
def main(
    ...
    """
    ICECODE Agent CLI - Interactive AI Assistant
    ...
    os.environ["ICECODE_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting ICECODE Gateway (messaging platforms)...")
    ...
    # so main unwinds normally.  ICECODE_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "ICECODE" box, no tool-gen
