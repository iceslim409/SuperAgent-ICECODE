"""``hermes debug`` debug tools for ICECODE Agent.
...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.icecode/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.icecode/pastes/pending.json``.
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, ICECODE version, provider, which API keys
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "icecode-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.icecode/pastes/pending.json``
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "icecode-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ICECODEDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from icecode_cli.hermes_cli.logs import LOG_FILES
...
def run_debug_share(args):
    ...
    print(f"\nShare these links with the ICECODE team for support.")
