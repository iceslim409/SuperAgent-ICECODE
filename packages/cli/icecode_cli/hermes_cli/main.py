...
Usage:
    ...
    hermes honcho migrate                  # ICECODE + Honcho
    ICECODE version             Show version
    hermes update              Update to latest version
    hermes uninstall           Uninstall ICECODE Agent
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ICECODE_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
#
# Many modules cache ICECODE_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ICECODE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.icecode/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ICECODE_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If ICECODE_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.icecode/profiles/coder or
    # /opt/data/profiles/coder).  If ICECODE_HOME points to the hermes root
    # instead (e.g. systemd hardcodes ICECODE_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("ICECODE_HOME", "")
    ...
    if profile_name is None:
        try:
            from icecode.icecode_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set ICECODE_HOME
    if profile_name is not None:
        try:
            from icecode_cli.hermes_cli.profiles import resolve_profile_env
            ...
            return
        os.environ["ICECODE_HOME"] = hermes_home
...
# Bridge security.redact_secrets from config.yaml → ICECODE_REDACT_SECRETS env
# var BEFORE hermes_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ICECODE_REDACT_SECRETS" not in os.environ:
        ...
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ICECODE_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
...
try:
    from icecode.icecode_logging import setup_logging as _setup_logging
...
try:
    from icecode_cli.hermes_cli.config import load_config as _load_config_early
    from icecode.icecode_constants import apply_ipv4_preference as _apply_ipv4
...
from icecode_cli.hermes_cli import __version__, __release_date__
from icecode.icecode_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from icecode_cli.hermes_cli.config import get_env_path, get_hermes_home, load_config
    from icecode_cli.hermes_cli.auth import get_auth_status

    # ICECODE itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from icecode_cli.hermes_cli.config import DEFAULT_CONFIG
    ...
    # ICECODE has been explicitly configured — Claude Code
    # ICECODE to use their tokens.
...
def _ensure_tui_node() -> None:
    ...
    ``ICECODE_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ICECODE_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    hermes_home = os.environ.get("ICECODE_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ICECODE_HOME": hermes_home},
            capture_output=True,
            text=True,
            check=False,
        )
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ICECODE_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ICECODE_NODE")
    ...
    ext_dir = os.environ.get("ICECODE_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ICECODE_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ICECODE_TUI_DIR (e.g. `unset ICECODE_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("ICECODE_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from icecode_cli.hermes_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env["ICECODE_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ICECODE_PYTHON_SRC_ROOT"] = os.environ.get(
        "ICECODE_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ICECODE_PYTHON", sys.executable)
    env.setdefault("ICECODE_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ICECODE_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ICECODE_MODEL"] = model
        env["ICECODE_INFERENCE_MODEL"] = model
    if provider:
        env["ICECODE_TUI_PROVIDER"] = provider
        env["ICECODE_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ICECODE_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ICECODE_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ICECODE_TUI_SKILLS"] = value
    if query:
        env["ICECODE_TUI_QUERY"] = query
    if image:
        env["ICECODE_TUI_IMAGE"] = image
    if checkpoints:
        env["ICECODE_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ICECODE_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ICECODE_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ICECODE_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ICECODE_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ICECODE_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["ICECODE_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ICECODE_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("ICECODE_KANBAN_BOARD"):
        return
    try:
        from icecode_cli.hermes_cli.kanban_db import get_current_board

        os.environ["ICECODE_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ICECODE_TUI") == "1"
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like ICECODE isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  hermes setup")
        print()

        from icecode_cli.hermes_cli.setup import (
    ...
    try:
        from icecode_cli.hermes_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ICECODE_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.icecode/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ICECODE_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ICECODE_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ICECODE_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from icecode_cli.hermes_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from icecode_cli.hermes_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with ICECODE?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ICECODE Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_setup(args):
    ...
    from icecode_cli.hermes_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    from icecode_cli.hermes_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from icecode_cli.hermes_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ICECODE_INFERENCE_PROVIDER") or "auto"
    )
    ...
    from icecode_cli.hermes_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from icecode_cli.hermes_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.icecode/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.icecode/.env if the active provider is not 'custom'.
    ...
    """
    from icecode_cli.hermes_cli.config import get_env_value, save_env_value, load_config
...
#
# ICECODE uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  ICECODE only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from icecode_cli.hermes_cli.config import load_config
    from icecode_cli.hermes_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from icecode_cli.hermes_cli.auth import _prompt_model_selection
    from icecode_cli.hermes_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from icecode_cli.hermes_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from icecode_cli.hermes_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.icecode/.env by hand. The
    ...
    from icecode_cli.hermes_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.icecode/.env by hand.
    ...
    from icecode_cli.hermes_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from icecode_cli.hermes_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from icecode_cli.hermes_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from icecode_cli.hermes_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from icecode_cli.hermes_cli.codex_models import get_codex_model_ids
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from icecode_cli.hermes_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from icecode_cli.hermes_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from icecode_cli.hermes_cli.auth import _save_model_choice, deactivate_provider
    from icecode_cli.hermes_cli.config import get_env_value, load_config, save_config
    ...
    from icecode_cli.hermes_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from icecode_cli.hermes_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use ICECODE URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from icecode_cli.hermes_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from icecode_cli.hermes_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from icecode_cli.hermes_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  ICECODE will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from icecode_cli.hermes_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from icecode_cli.hermes_cli.config import save_env_value, load_config, save_config
    from icecode_cli.hermes_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from icecode_cli.hermes_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from icecode_cli.hermes_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates ICECODE turns to `copilot --acp`.")
    print("  ICECODE currently starts its own ACP subprocess for each request.")
    print("  ICECODE uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ICECODE_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.icecode/.env`` by hand.
    ...
    from icecode_cli.hermes_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   ICECODE typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with ICECODE, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from icecode_cli.hermes_cli.auth import AuthError
        from icecode_cli.hermes_cli.models import fetch_lmstudio_models
        ...
        from icecode_cli.hermes_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from icecode.icecode_constants import display_hermes_home as _dhh_fn

            print(
                f"    ICECODE will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from icecode_cli.hermes_cli.auth import get_anthropic_key
...
def cmd_login(args):
    """Authenticate ICECODE CLI with a provider."""
    from icecode_cli.hermes_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from icecode_cli.hermes_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from icecode_cli.hermes_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from icecode_cli.hermes_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from icecode_cli.hermes_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from icecode_cli.hermes_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from icecode_cli.hermes_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from icecode_cli.hermes_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from icecode_cli.hermes_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from icecode_cli.hermes_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from icecode_cli.hermes_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from icecode_cli.hermes_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from icecode_cli.hermes_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up ICECODE home directory to a zip file."""
    if getattr(args, "quick", False):
        from icecode_cli.hermes_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from icecode_cli.hermes_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a ICECODE backup from a zip file."""
    from icecode_cli.hermes_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"ICECODE Agent v{__version__} ({__release_date__})")
    ...
    try:
        from icecode_cli.hermes_cli.banner import check_for_updates
        from icecode_cli.hermes_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall ICECODE Agent."""
    _require_tty("uninstall")
    from icecode_cli.hermes_cli.uninstall import run_uninstall
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://icecode-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    """Update ICECODE Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/icecode-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
        zip_path = os.path.join(tmp_dir, f"icecode-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to icecode-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"icecode-agent-{branch}")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if ICECODE behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but ICECODE couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but ICECODE couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if ICECODE behaves unexpectedly.")
    return True
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/icecode-agent.git",
    "git@github.com:NousResearch/icecode-agent.git",
    "https://github.com/NousResearch/icecode-agent",
    "git@github.com:NousResearch/icecode-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/icecode-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official ICECODE repository.")
        print("  This means you may miss updates from NousResearch/icecode-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/icecode-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/icecode-agent.git' to add later."
            )
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.icecode/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.icecode/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        from icecode_cli.hermes_cli.config import get_hermes_home as _get_hermes_home
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/icecode-agent).
    ...
    path_comment = (
        "# ICECODE Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ICECODE_HOME before running the update.
    ...
    ICECODE_HOME directories.  The ``--backup`` flag on ``hermes update``
    ...
    try:
        from icecode_cli.hermes_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_hermes_home so the user sees ~/.icecode/...
    try:
...
def cmd_update(args):
    """Update ICECODE Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from icecode_cli.hermes_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update ICECODE Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/icecode-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $ICECODE_HOME, this is cheap
        # belt-and-suspenders insurance and gives the user something to
        # restore from via `/snapshot list` / `/snapshot restore <id>`.
        try:
            from icecode_cli.hermes_cli.backup import create_quick_snapshot
        ...
        try:
            import importlib
            import icecode.icecode_constants as icecode_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ICECODE_HOME so
        # it is not affected by sync_skills()'s module-level ICECODE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ICECODE_HOME env var points at the default or a named profile.
        try:
            from icecode_cli.hermes_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from icecode_cli.hermes_cli.tools_config import install_cua_driver
        ...
        try:
            from icecode_cli.hermes_cli.gateway import (
                is_macos,
            ...
            try:
                from icecode.icecode_constants import (
        ...
        # ICECODE gateway unit files are still installed.
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy ICECODE gateway unit(s) detected:")
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.icecode)")
            else:
        ...
        from icecode_cli.hermes_cli.profiles import profile_exists
        ...
        from icecode_cli.hermes_cli.profiles import rename_profile
        ...
        from icecode_cli.hermes_cli.profiles import export_profile
        ...
        from icecode_cli.hermes_cli.profiles import import_profile
        ...
        import tempfile
        from icecode_cli.hermes_cli.profile_distribution import (
            plan_install,
        ...
        from icecode_cli.hermes_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from icecode_cli.hermes_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     ICECODE {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from icecode_cli.hermes_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hermes_requires:
        print(f"  Requires: ICECODE {mf.hermes_requires}")
...
def cmd_dashboard(args):
    ...
    if "ICECODE_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ICECODE_WEB_DIST"])
            if "ICECODE_WEB_DIST" in os.environ
    ...
    from icecode_cli.hermes_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ICECODE_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from icecode_cli.hermes_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter ICECODE log files."""
    from icecode_cli.hermes_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from icecode_cli.hermes_cli.models import CANONICAL_PROVIDERS as _cp
...
def main():
    ...
    from icecode_cli.hermes_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from icecode_cli.hermes_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://icecode-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        ...
        description=(
            "Stop, disable, and remove legacy ICECODE gateway unit files "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure ICECODE Agent with an interactive wizard. "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for ICECODE.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ICECODE_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "ICECODE")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for ICECODE CLI",
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate ICECODE with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ICECODE_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of ICECODE Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.icecode/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.icecode/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from icecode_cli.hermes_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.icecode/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.icecode/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with ICECODE Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your ICECODE setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for ICECODE Agent. Use 'hermes debug share' to "
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up ICECODE home directory to a zip file",
        description="Create a zip archive of your entire ICECODE configuration, "
        "skills, sessions, and data (excludes the icecode-agent codebase). "
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.icecode/checkpoints/",
    ...
    from icecode_cli.hermes_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a ICECODE backup from a zip file",
        description="Extract a previously created ICECODE backup into your "
        "ICECODE home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage ICECODE Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from icecode_cli.hermes_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.icecode/skills/.bundled_manifest) "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from icecode_cli.hermes_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from icecode_cli.hermes_cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from icecode_cli.hermes_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``hermes --help``, ``ICECODE version``, ``hermes logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from icecode_cli.hermes_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from icecode_cli.hermes_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from icecode_cli.hermes_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from icecode_cli.hermes_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from icecode_cli.hermes_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run ICECODE as an MCP server",
        description=(
            "Manage MCP server connections and run ICECODE as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'hermes mcp add' to connect to a new server, or\n"
            "'hermes mcp serve' to expose ICECODE conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run ICECODE as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from icecode_cli.hermes_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from icecode_cli.hermes_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to ICECODE",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to ICECODE",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.icecode/ (by default a "
        "single restore-point archive is written to ~/.icecode/backups/ "
    ...
    def cmd_claw(args):
        from icecode_cli.hermes_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update ICECODE Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall ICECODE Agent",
        description="Remove ICECODE Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run ICECODE Agent as an ACP (Agent Client Protocol) server",
        description="Start ICECODE Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch ICECODE Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated ICECODE instances",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a ICECODE profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the ICECODE Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        help=(
            ...
            "Alternatively set ICECODE_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter ICECODE log files",
...
""",
    ...
    from icecode_cli.hermes_cli.config import get_container_exec_info
    ...
    if getattr(args, "oneshot", None):
        from icecode_cli.hermes_cli.oneshot import run_oneshot
