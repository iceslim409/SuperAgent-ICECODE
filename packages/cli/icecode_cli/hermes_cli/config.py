"""
Configuration management for ICECODE Agent.

Config files are stored in ~/.icecode/ for easy access:
- ~/.icecode/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.icecode/.env         - API keys and secrets
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.icecode/config.yaml`` causes ``load_config()``
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ICECODE_LANGFUSE_ENV",
    "ICECODE_LANGFUSE_RELEASE",
    "ICECODE_LANGFUSE_SAMPLE_RATE",
    "ICECODE_LANGFUSE_MAX_CHARS",
    "ICECODE_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from icecode_cli.hermes_cli.colors import Colors, color
from icecode_cli.hermes_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ICECODE_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the ICECODE_MANAGED env var (set by the systemd service),
    or a .managed marker file in ICECODE_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade icecode-agent"
...
def format_managed_message(action: str = "modify this ICECODE installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ICECODE_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this ICECODE installation is managed by NixOS "
            f"(ICECODE_MANAGED={env_hint}).\n"
            "Edit services.icecode-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this ICECODE installation is managed by Homebrew "
            f"(ICECODE_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade icecode-agent"
        )

    return (
        f"Cannot {action}: this ICECODE installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall ICECODE."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ICECODE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or ICECODE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ICECODE_DEV") == "1":
        return None

    from icecode.icecode_constants import is_container
    if is_container():
        return None
    ...
    container_name = info.get("container_name", "icecode-agent")
...
def _secure_dir(path):
    ...
    The mode can be overridden via the ICECODE_HOME_MODE environment variable
    (e.g. ICECODE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ICECODE_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ICECODE_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("ICECODE_CONTAINER") or os.environ.get("ICECODE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ICECODE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ICECODE_HOME if the user doesn't have one yet."""
...
def ensure_hermes_home():
    """Ensure ~/.icecode directory structure exists with secure permissions.
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"ICECODE_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # ICECODE-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # ICECODE auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # ICECODE sources the user's shell rc files
        ...
        #  "/home/user/.icecode/cache/documents:/output"]
        ...
        # ICECODE image, which drops to the `hermes` user via gosu).
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.icecode/checkpoints/`` size (MB).  When
    ...
    "display": {
        ...
        # users aren't surprised.  ICECODE_TUI_RESUME=<id> always wins.
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.icecode/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.icecode/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # ICECODE feeds a continuation prompt back into the
    ...
    "goals": {
        # ICECODE auto-pauses the goal and
    ...
    # always goes to ~/.icecode/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ICECODE_SKILL_DIR} and ${ICECODE_SESSION_ID} in SKILL.md
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.icecode/skills/ into
        # ~/.icecode/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ICECODE Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ICECODE_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.icecode/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ICECODE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        "acked_advisories": [],
        # ICECODE to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ICECODE_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     icecode-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.icecode/logs/.
    ...
    # update model picker lists without shipping a icecode-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://icecode-agent.nousresearch.com/docs/api/model-catalog.json",
    ...
    # Session storage — controls automatic cleanup of ~/.icecode/state.db.
    ...
    "updates": {
        # Run a full ``hermes backup``-style zip of ICECODE_HOME before every
        # ``hermes update``.  Backups land in ``<ICECODE_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default —
        # on large ICECODE_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<ICECODE_HOME>/lsp/bin/`` on first use.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the ICECODE auth store)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ICECODE_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'icecode-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote ICECODE API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote ICECODE API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote ICECODE API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ICECODE_TOOL_PROGRESS and ICECODE_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "ICECODE_TOOL_PROGRESS": {
    ...
    "ICECODE_EPHEMERAL_SYSTEM_PROMPT": {
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older ICECODE versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — ICECODE won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ICECODE_HOME", "~/.icecode")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ICECODE_TOOL_PROGRESS")
            old_mode = get_env_value("ICECODE_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ICECODE_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ICECODE_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ICECODE_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ICECODE_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ICECODE_HOME/plugins/`` for currently installed user plugins.
    ...
    #   3. Creates `~/.icecode/logs/curator/` if missing (belt-and-suspenders
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.icecode/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.icecode/config.yaml.
    ...
    (which change ``ICECODE_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_hermes_home()
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.icecode/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.icecode/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.icecode/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.icecode/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.icecode/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.icecode/.env into os.environ. Returns count of vars updated.
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # ICECODE vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.icecode/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ ICECODE Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
