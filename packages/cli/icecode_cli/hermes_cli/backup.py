...
`hermes backup` creates a zip archive of the entire ~/.icecode/ directory
(excluding the icecode-agent repo and transient files).
...
from icecode.icecode_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
...
_EXCLUDED_DIRS = {
    "icecode-agent",     # the codebase repo — re-clone instead
...
def run_backup(args) -> None:
    """Create a zip backup of the ICECODE home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: ICECODE home directory not found at {hermes_root}")
        sys.exit(1)
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    if not found:
        return False, (
            "zip does not appear to be a ICECODE backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.icecode/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has ICECODE configuration.")
        ...
        # Guidance
        print()
        if not (hermes_root / "icecode-agent").is_dir():
            print("Note: The icecode-agent codebase was not included in the backup.")
        ...
        print("Done. Your ICECODE configuration has been restored.")
...
# Critical state files to include in quick snapshots (relative to ICECODE_HOME).
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ICECODE_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ICECODE_HOME>/backups/pre-update-<timestamp>.zip`` and
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ICECODE_HOME under ``backups/`` before a
    ...
    ``<ICECODE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
