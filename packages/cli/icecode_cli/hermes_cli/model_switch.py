...
_ICECODE_MODEL_WARNING = (
    "Nous Research ICECODE 3 & 4 models are NOT agentic and are not designed "
    "for use with ICECODE Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# ICECODE 4 chat families.
...
# ICECODE-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   hermes-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ICECODE_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_ICECODE_NON_AGENTIC_RE.search(model_name))


def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous ICECODE 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _ICECODE_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from icecode_cli.hermes_cli.model_switch import
...
def list_authenticated_providers(
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from icecode_cli.hermes_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://icecode-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # ICECODE release. Falls back to the in-repo
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (ICECODE_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from icecode_cli.hermes_cli.auth import _load_auth_store
    ...
    # ICECODE-only providers (nous, openai-codex, copilot, opencode-go) ---
    from icecode_cli.hermes_cli.providers import ICECODE_OVERLAYS
    from icecode_cli.hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # ICECODE provider ID.
    # ICECODE_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # ICECODE IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ICECODE_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # ICECODE slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # ICECODE slug, fall back to overlay key
            model_ids = curated.get(hermes_slug, []) or curated.get(pid, [])
    ...
    # in PROVIDER_TO_MODELS_DEV or ICECODE_OVERLAYS (keeps /model in sync
    # with `hermes model`).
    try:
        from icecode_cli.hermes_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ICECODE's canonical write key (matches
            ...
            # ICECODE writes ``models:`` as a dict keyed by model id
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # ICECODE's own writer uses; a hyphen variant is accepted
            ...
            # ICECODE's own writer (main.py::_save_custom_provider)
...
def list_picker_providers(
    ...
    """
    from icecode_cli.hermes_cli.models import fetch_openrouter_models
