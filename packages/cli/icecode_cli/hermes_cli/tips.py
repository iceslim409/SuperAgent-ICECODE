...
TIPS = [
    ...
    "Ctrl+Z suspends ICECODE to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs ICECODE itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire ICECODE home directory.",
    ...
    "hermes gateway install sets up ICECODE as a system service (systemd/launchd).",
    ...
    "ICECODE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call ICECODE tools programmatically — results stay out of context.",
    ...
    "When exiting, ICECODE prints a resume command with session ID and stats.",
    ...
    "The plan skill saves markdown plans under .icecode/plans/ in the active workspace.",
    ...
    # --- Gateway & Messaging ---
    "ICECODE runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to ICECODE via a local macOS server.",
    ...
    "SOUL.md at ~/.icecode/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "ICECODE loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "Checkpoints use shadow repos in ~/.icecode/checkpoints/ — your project's .git is never touched.",
    ...
    "hermes acp runs ICECODE as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ICECODE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "Cron scripts live in ~/.icecode/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make ICECODE your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.icecode/SOUL.md to customize.",
    ...
    "Before a gateway session reset, ICECODE auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.icecode/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ICECODE_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ICECODE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ICECODE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.icecode/interrupt_debug.log with timestamps.",
    ...
    "ICECODE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hermes tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.icecode/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.icecode/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — ICECODE auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of ICECODE config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while ICECODE is working.',
    ...
    '/reload reloads ~/.icecode/.env into the running session — pick up new API keys without restarting.',
    ...
    "ICECODE_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.icecode/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.icecode/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    # --- TUI & Dashboard ---
    'ICECODE_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ICECODE_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'hermes dashboard --tui embeds the full ICECODE TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.icecode/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.icecode/dashboard-plugins/ — no npm build required.',
    ...
    'ICECODE_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ICECODE_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'ICECODE_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ICECODE_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.icecode/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.icecode/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hermes chat --ignore-user-config skips ~/.icecode/config.yaml — reproducible bug reports and CI runs.',
    ...
    'ICECODE_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ICECODE_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.icecode/plugins/ — trust-gated by design.',
    "ICECODE_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ICECODE_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ICECODE_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ICECODE_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ICECODE_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ICECODE_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ICECODE_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ICECODE_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ICECODE_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ICECODE_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ICECODE_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes ICECODE block commands when the tirith scanner itself errors out.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.icecode/dashboard-plugins/.',
]
