...
_ICECODE_USER_AGENT = f"hermes-cli/{_ICECODE_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from icecode_cli.hermes_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ICECODE_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ICECODE_HOME/models_dev_cache.json directly (no network) so this
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from icecode_cli.hermes_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    icecode-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from icecode_cli.hermes_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — icecode-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from icecode_cli.hermes_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from icecode.icecode_constants import AI_GATEWAY_BASE_URL
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from icecode_cli.hermes_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from icecode_cli.hermes_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether ICECODE should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.icecode/.env``.
    ...
    """
    try:
        from icecode_cli.hermes_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from icecode_cli.hermes_cli.auth import read_credential_pool
        from icecode_cli.hermes_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "openai-codex":
        ...
        # ICECODE release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from icecode_cli.hermes_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ICECODE_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # ICECODE' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ICECODE_USER_AGENT}
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"ICECODE will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so ICECODE cannot verify the model name."
