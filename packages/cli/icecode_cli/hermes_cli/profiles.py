...
Each profile is a fully independent ICECODE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.icecode/profiles/<name>/`` by default.

The "default" profile is ``~/.icecode`` itself — backward compatible,
...
# default profile (``~/.icecode``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   icecode-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.icecode``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.icecode) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "icecode-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# ICECODE subcommands that cannot be used as profile names/aliases
_ICECODE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current ICECODE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ICECODE_HOME points outside
    ``~/.icecode``, profiles live under ``ICECODE_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) ICECODE_HOME path.

    In standard deployments this is ``~/.icecode``.
    In Docker/custom deployments where ICECODE_HOME is outside ``~/.icecode``
    (e.g. ``/opt/data``), returns ICECODE_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the ICECODE installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ICECODE_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _ICECODE_SUBCOMMANDS:
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.icecode)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.icecode/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ICECODE_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ICECODE_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.icecode).\n"
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ICECODE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ICECODE_HOME")
    try:
        os.environ["ICECODE_HOME"] = str(profile_dir)
        from icecode_cli.hermes_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ICECODE_HOME"] = old_home
        elif "ICECODE_HOME" in os.environ:
            del os.environ["ICECODE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.icecode/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ICECODE_HOME.

    Returns ``"default"`` if ICECODE_HOME is not set or points to ``~/.icecode``.
    Returns the profile name if ICECODE_HOME points into ``~/.icecode/profiles/<name>``.
    Returns ``"custom"`` if ICECODE_HOME is set to an unrecognized path.
    """
    from icecode.icecode_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.icecode itself — its parent is ~/ and its
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.icecode itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.icecode). "
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ICECODE_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the ICECODE_HOME environment variable.
    """
