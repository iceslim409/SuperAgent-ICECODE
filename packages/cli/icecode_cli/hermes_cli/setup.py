"""
Interactive setup wizard for ICECODE Agent.
...
_DOCS_BASE = "https://icecode-agent.nousresearch.com/docs"
...
from icecode_cli.hermes_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ ICECODE Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure ICECODE using environment variables or config commands:")
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from icecode_cli.hermes_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from icecode_cli.hermes_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from icecode_cli.hermes_cli.plugins import _ensure_plugins_discovered
    ...
    try:
        ...
        from icecode_cli.hermes_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    try:
        from icecode_cli.hermes_cli.auth import get_provider_auth_state
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from icecode_cli.hermes_cli.main import select_provider_and_model
    try:
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from icecode_cli.hermes_cli.auth_commands import auth_add_command
            ...
            print_info(
                "ICECODE can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one ICECODE can auto-use for vision,")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.icecode/.env."
            )
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where ICECODE runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'icecode-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'icecode-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ICECODE_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ICECODE_MAX_ITERATIONS")
    ...
    # ICECODE Agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ICECODE_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where ICECODE delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://icecode-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ICECODE_HOME, and print
    ...
    """
    try:
        from icecode_cli.hermes_cli.slack_cli import _build_full_manifest
        from icecode.icecode_constants import get_hermes_home

        manifest = _build_full_manifest(
            bot_name="ICECODE",
            bot_description="Your ICECODE agent on Slack",
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where ICECODE delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where ICECODE delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects ICECODE to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from icecode_cli.hermes_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://icecode-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://icecode-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  hermes config edit")
    print_info("   Open config in your editor:  hermes config edit")
...
def setup_gateway(config: dict):
    ...
    from icecode_cli.hermes_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with ICECODE from anywhere.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from icecode_cli.hermes_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from icecode_cli.hermes_cli.config
    ...
    if section_key == "model":
        ...
        from icecode_cli.hermes_cli.gateway import _all_platforms, _platform_status
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure ICECODE to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point ICECODE at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point ICECODE at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point ICECODE at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point ICECODE at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to ICECODE equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing ICECODE config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in ICECODE.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ ICECODE's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("ICECODE can preview what would be imported before making any changes.")
    print()
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # ICECODE config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in ICECODE (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ ICECODE Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ ICECODE Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your ICECODE Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have ICECODE configured.")
        ...
        setup_mode = prompt_choice("How would you like to set up ICECODE?", [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect ICECODE to messaging apps to chat from anywhere.")
