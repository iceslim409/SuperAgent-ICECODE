"""
ICECODE Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.icecode/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove ICECODE PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing icecode-agent or hermes PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# ICECODE Agent" comment and following line
                if '# ICECODE Agent' in line or '# icecode-agent' in line:
                    skip_next = True
                    continue
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'icecode-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from icecode_cli.hermes_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from icecode_cli.hermes_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ICECODE_HOME`` and ``ICECODE_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify ICECODE-owned User-PATH entries."""
    root = str(hermes_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare icecode-agent install dir.
    markers = [root + "\\icecode-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ICECODE_HOME was customised to somewhere else — find-and-nuke
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete ICECODE_HOME and ICECODE_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ICECODE_HOME", "ICECODE_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ICECODE_HOME directory.
    ...
    current ICECODE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's ICECODE_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.icecode/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.icecode/ for future reinstall
    """
    project_root = get_project_root()
    hermes_home = get_hermes_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ICECODE_HOMEs
    ...
    print(color("│            ⚕ ICECODE Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their ICECODE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL ICECODE data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the ICECODE code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No ICECODE-owned PATH entries in User environment")

        log_info("Removing ICECODE_HOME / ICECODE_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No ICECODE-set User env vars to remove")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.icecode/, just remove the icecode-agent subdir
    ...
    #     under ICECODE_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.icecode/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ICECODE_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/NousResearch/icecode-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/icecode-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using ICECODE Agent! ⚕")
    print()
