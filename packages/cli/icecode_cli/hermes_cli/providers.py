"""
Single source of truth for provider identity in ICECODE Agent.
...
# ICECODE overlay ----------------------------------------------------------
# ICECODE-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """ICECODE-specific provider metadata layered on top of models.dev."""
...
ICECODE_OVERLAYS: Dict[str, HermesOverlay] = {
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="ICECODE_QWEN_BASE_URL",
    ),
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    overlay = ICECODE_OVERLAYS.get(canonical)
    ...
    if overlay is not None:
        # ICECODE-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ICECODE_OVERLAYS
    if provider == "bedrock":
