"""Helpers for loading ICECODE .env files consistently across entrypoints."""
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from icecode_cli.hermes_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.icecode/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("ICECODE_HOME", Path.home() / ".hermes"))
