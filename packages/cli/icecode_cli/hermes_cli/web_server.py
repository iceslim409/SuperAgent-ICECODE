...
WEB_DIST = Path(os.environ["ICECODE_WEB_DIST"]) if "ICECODE_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="ICECODE Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-ICECODE-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``hermes dashboard --tui``
# or ICECODE_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.icecode/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ICECODE_NONINTERACTIVE": "1"},
    }
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.icecode/config.yaml`` — applies to **new** sessions only.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.icecode/.anthropic_oauth.json`` — Hermes-managed PKCE flow
    ...
    try:
        ...
        _ICECODE_OAUTH_FILE = None  # type: ignore
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"ICECODE PKCE ({_ICECODE_OAUTH_FILE})",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from icecode_cli.hermes_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # ICECODE-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _ICECODE_OAUTH_FILE
            if _ICECODE_OAUTH_FILE.exists():
                _ICECODE_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from icecode_cli.hermes_cli.auth import clear_provider_auth
...
#          → persists to ~/.icecode/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _ICECODE_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _ICECODE_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from icecode_cli.hermes_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("ICECODE_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from icecode_cli.hermes_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from icecode_cli.hermes_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from icecode_cli.hermes_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from icecode.icecode_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from icecode_cli.hermes_cli import profiles as profiles_mod
    try:
...
async def get_skills():
    ...
    from icecode_cli.hermes_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from icecode_cli.hermes_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from icecode_cli.hermes_cli.tools_config import (
...
try:
    from icecode_cli.hermes_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``ICECODE_TUI_RESUME`` env var —
    matching what ``hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``ICECODE_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from icecode_cli.hermes_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ICECODE_TUI_DISABLE_MOUSE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ICECODE_TUI_RESUME"] = resume

    if sidecar_url:
        env["ICECODE_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall ICECODE inside WSL2 to use the dashboard's /chat "
...
# ICECODE_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__ICECODE_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        token_script = (
            f'<script>window.__ICECODE_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__ICECODE_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__ICECODE_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # ICECODE backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "ICECODE Teal",         "description": "Classic dark teal — the canonical ICECODE look"},
    {"name": "default-large", "label": "ICECODE Teal (Large)", "description": "ICECODE Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.icecode/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.icecode/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.icecode/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.icecode/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.icecode/plugins/  (only if ICECODE_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from icecode_cli.hermes_cli.plugins import get_bundled_plugins_dir
    ...
    if os.environ.get("ICECODE_ENABLE_PROJECT_PLUGINS"):
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from icecode_cli.hermes_cli.plugins_cmd import (
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from icecode_cli.hermes_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from icecode_cli.hermes_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from icecode_cli.hermes_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from icecode_cli.hermes_cli.plugins_cmd import dashboard_remove_user_plugin
...
def start_server(
    ...
    *,
    ...
    print(f"  ICECODE Web UI → http://{host}:{port}")
