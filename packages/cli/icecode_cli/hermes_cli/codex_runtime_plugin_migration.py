...
MIGRATION_MARKER = (
    "# managed by icecode-agent — `hermes codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end icecode-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in ICECODE config.")
...
# ICECODE keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # ICECODE' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    if has_command:
        ...
        # ICECODE' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_ICECODE_KEYS:
            skipped.append(f"{key} (unknown ICECODE key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ICECODE_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by ICECODE)")
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    ICECODE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ICECODE_HOME passes through if set so the MCP subprocess sees the
    # same config / auth / sessions DB as the parent CLI.
    hermes_home = os.environ.get("ICECODE_HOME")
    if hermes_home:
        env["ICECODE_HOME"] = hermes_home
    ...
    env["ICECODE_QUIET"] = "1"
    env["ICECODE_REDACT_SECRETS"] = env.get("ICECODE_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.icecode/config.yaml dict
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in ICECODE config is not a dict; cannot migrate."
        )
        return report
    ...
    # ICECODE' own tool surface as an MCP server so the spawned
    # ICECODE for the tools codex
