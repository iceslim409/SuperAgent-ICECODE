...
_TASK_NAME_DEFAULT = "ICECODE_Gateway"
_TASK_DESCRIPTION = "ICECODE Agent Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    from icecode_cli.hermes_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<ICECODE_HOME>/gateway-service/<task_name>.cmd`` so per-profile
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ICECODE_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    lines.append(f'set "ICECODE_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ICECODE_GATEWAY_DETACHED=1"')
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    env_overlay = {
        "ICECODE_HOME": hermes_home,
        "PYTHONIOENCODING": "utf-8",
        "ICECODE_GATEWAY_DETACHED": "1",
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from icecode_cli.hermes_cli.gateway import find_gateway_pids
...
def stop() -> None:
    ...
    _assert_windows()
    from icecode_cli.hermes_cli.gateway import kill_gateway_processes
