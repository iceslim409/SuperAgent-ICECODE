...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "icecode-agent", ".worktrees", "profiles", "bin", "node_modules",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires ICECODE {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this ICECODE distribution.",
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a ICECODE profile distribution."
            )
...
def plan_install(
    ...
    from icecode_cli.hermes_cli import __version__ as hermes_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a ICECODE distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.icecode).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
