...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with icecode-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.icecode/plugins/<name>/``
3. **Project plugins** – ``./.icecode/plugins/<name>/`` (opt-in via
   ``ICECODE_ENABLE_PROJECT_PLUGINS``)
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``ICECODE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ICECODE_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ICECODE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.icecode/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ICECODE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ICECODE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "ICECODE_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManifest:
    ...
    #              in ~/.icecode/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def register_skill(
        ...
        ``~/.icecode/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        # 2. User plugins (~/.icecode/plugins/)
        ...
        # 3. Project plugins (./.icecode/plugins/)
        if _env_enabled("ICECODE_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set ICECODE_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
