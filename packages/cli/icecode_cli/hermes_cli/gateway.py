    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"ICECODE_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ICECODE_HOME
        ...
        # ICECODE_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "ICECODE_HOME=" in command and f"ICECODE_HOME={current_home}" not in command:
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to ICECODE profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from icecode_cli.hermes_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ICECODE_HOME``.

    Under ``sudo``, ``ICECODE_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``ICECODE_HOME`` for the actual gateway
    ...
    unit_home = env.get("ICECODE_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ICECODE_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ICECODE_HOME"] = unit_home
...
def _gateway_list() -> None:
    ...
    """
    try:
        from icecode_cli.hermes_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ICECODE_HOME-scoped).
...
from icecode.icecode_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    ``ICECODE_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ICECODE_GATEWAY_DETACHED", "").strip().lower()
...
SERVICE_DESCRIPTION = "ICECODE Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ICECODE_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ICECODE_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ICECODE_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ICECODE_HOME is a named profile.

    For ``~/.icecode/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit ICECODE_HOME path. Defaults to the current
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ICECODE_HOME.

    Default ``~/.icecode`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.icecode/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other ICECODE_HOME appends a short hash for uniqueness.
    """
...
# ICECODE installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    ...
    Detects unit files installed by older ICECODE versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy ICECODE gateway unit files exist."""
...
def print_legacy_unit_warning() -> None:
    ...
    if not legacy:
        return
    print_warning("Legacy ICECODE gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy ICECODE gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy ICECODE gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so ICECODE can't create it from this user session.")
            if run_as_user:
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.icecode`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.icecode/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.icecode/icecode-agent  -> /home/alice/.icecode/icecode-agent
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ICECODE_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.icecode/profiles/coder     → /home/alice/.icecode/profiles/coder
    ...
    # Default ~/.icecode → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.icecode → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.icecode) — keep as-is
...
Environment="ICECODE_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ICECODE_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ICECODE_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="ICECODE_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current ICECODE install")
    return True
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current ICECODE install")
    return True
...
def launchd_install(force: bool = False):
    ...
    from icecode.icecode_constants import display_hermes_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current ICECODE install")
    else:
        print("⚠ Service definition is stale relative to the current ICECODE install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ICECODE_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the ICECODE gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the ICECODE command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ICECODE_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set ICECODE_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # Service-style launchers set ICECODE_GATEWAY_DETACHED=1; older wrappers
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ ICECODE Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # ICECODE_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ICECODE_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from icecode.icecode_constants import get_hermes_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and ICECODE will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where ICECODE delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your ICECODE agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address ICECODE will use (e.g., hermes@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. ICECODE connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and ICECODE will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.icecode/plugins/ still require
...
def _setup_whatsapp():
    ...
    from icecode_cli.hermes_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from icecode_cli.hermes_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from icecode_cli.hermes_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. ICECODE will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. ICECODE will store the returned account_id/token in ~/.icecode/.env.")
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"ICECODEAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    """
    from icecode_cli.hermes_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.icecode/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.icecode/.env: {', '.join(required)}")
    else:
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("  nohup hermes gateway run > ~/.icecode/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup hermes gateway run > ~/.icecode/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'hermes gateway run'         # persistent via tmux")
                    print("  nohup hermes gateway run > ~/.icecode/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
        ...
        # ICECODE gateway unit files from
