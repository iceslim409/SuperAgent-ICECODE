...
Diagnoses issues with ICECODE Agent setup.
"""
...
from icecode_cli.hermes_cli.config import get_project_root, get_hermes_home, get_env_path
...
ICECODE_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.icecode or ~/.icecode/profiles/coder)

# Load environment variables from ~/.icecode/.env so API key checks work
...
from icecode.icecode_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.icecode/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("ICECODE_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ICECODE_KANBAN_TASK"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from icecode_cli.hermes_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    os.environ.setdefault("ICECODE_INTERACTIVE", "1")
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.icecode/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 ICECODE Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.icecode/.env (primary location for user config)
    env_path = ICECODE_HOME / '.env'
    if env_path.exists():
    ...
    # Check ~/.icecode/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ICECODE_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from icecode_cli.hermes_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from icecode_cli.hermes_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in {"auto", "custom", "openrouter"}:
                try:
                    from icecode_cli.hermes_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.icecode/.env or run 'hermes setup')",
                            )
                            issues.append(
    ...
    if config_path.exists():
        try:
            from icecode_cli.hermes_cli.config import check_config_version, migrate_config
    ...
    if _safe_which("codex"):
        ...
        # ICECODE' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give ICECODE a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# ICECODE Agent Persona\n\n"
                "<!-- Edit this file to customize how ICECODE communicates. -->\n\n"
                "You are ICECODE, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'icecode-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'icecode-agent[vercel]'")
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _ICECODE_USER_AGENT,
            }
    ...
    hub_dir = ICECODE_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ICECODE_HOME / "config.yaml"
    ...
    # =========================================================================
    try:
        from icecode_cli.hermes_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
