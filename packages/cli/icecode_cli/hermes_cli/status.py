...
Shows the status of all ICECODE Agent components.
"""
...
from icecode_cli.hermes_cli.auth import AuthError, resolve_provider
from icecode_cli.hermes_cli.colors import Colors, color
from icecode_cli.hermes_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from icecode_cli.hermes_cli.models import provider_label
from icecode_cli.hermes_cli.nous_subscription import get_nous_subscription_features
from icecode_cli.hermes_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all ICECODE Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ ICECODE Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if _effective_provider_label() == "LM Studio":
        from icecode_cli.hermes_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'icecode-agent[vercel]')"
    ...
    try:
        from icecode_cli.hermes_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
