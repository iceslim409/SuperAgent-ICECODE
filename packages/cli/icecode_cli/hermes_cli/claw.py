...
from icecode_cli.hermes_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "ICECODE may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print()
    print_error(
        "ICECODE gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to ICECODE")
...
def _cmd_migrate(args):
    """Run the OpenClaw → ICECODE migration."""
    ...
    # ICECODE gateway is running with connected platforms.
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.icecode/. "
    ...
    if not no_backup:
        try:
            from icecode_cli.hermes_cli.backup import create_pre_migration_backup, _format_size
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the ICECODE home."
            )
