...
Consent records live under ``~/.icecode/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.icecode/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.icecode/config.yaml.")
...
def _cmd_test(args) -> None:
    from icecode_cli.hermes_cli.config import load_config
    from icecode_cli.hermes_cli.plugins import VALID_HOOKS
    from agent import shell_hooks
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (ICECODE wire shape): {json.dumps(parsed)}")
    else:
