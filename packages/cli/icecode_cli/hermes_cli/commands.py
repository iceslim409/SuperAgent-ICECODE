"""Slash command definitions and autocomplete for the ICECODE CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of ICECODE config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal ICECODE works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while ICECODE is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.icecode/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update ICECODE Agent to the latest version", "Info",
               gateway_only=True),
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to ICECODE or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
...
def slack_app_manifest(request_url: str = "https://icecode-agent.local/slack/commands") -> dict[str, Any]:
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from icecode_cli.hermes_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
