...
def configure_windows_stdio() -> bool:
    ...
    Set ``ICECODE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ICECODE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # ICECODE-managed Git install directories so
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and ICECODE's
...
def _augment_path_with_known_tools() -> None:
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    candidate_dirs = [
        ...
        # ICECODE venv Scripts directory — host of the hermes.exe shim itself,
        ...
        os.path.join(local_appdata, "hermes", "icecode-agent", "venv", "Scripts"),
