...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@icecode/sdk/v2"
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ICECODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to ICECODE_SERVER_USERNAME or 'icecode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      try {
        const healthRes = await fetch(`${icecodePythonUrl}/health`)
        if (healthRes.ok) {
          sdk = createOpencodeClient({ baseUrl: icecodePythonUrl, directory })
        } else {
          throw new Error("not ready")
        }
      } catch {
        // Fallback la server-ul TS intern
        sdk = createOpencodeClient({
          baseUrl: "http://icecode.internal",
          fetch: fetchFn,
          directory,
        })
      }
