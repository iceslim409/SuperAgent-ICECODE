...
- GET  /v1/models                  — lists icecode-agent as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to icecode-agent
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted ICECODE_HOME (same filesystem
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through icecode-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """
        raw = request.headers.get("X-ICECODE-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-ICECODE-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-ICECODE-Session-Key requires API key authentication. "
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "icecode-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return icecode-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every ICECODE version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side ICECODE AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-ICECODE-Session-Id",
                "session_key_header": "X-ICECODE-Session-Key",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # ICECODE-Session-Key.  This
        # ICECODE-Session-Id: the key persists across
        ...
        provided_session_id = request.headers.get("X-ICECODE-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-ICECODE-Session-Id rejected: "
            ...
            # ICECODE session.  The first user
        ...
        response_headers = {
            "X-ICECODE-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-ICECODE-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-ICECODE-Completed"] = "false"
            response_headers["X-ICECODE-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # ICECODE-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-ICECODE-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-ICECODE-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-ICECODE-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-ICECODE-Session-Key"] = gateway_session_key
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-ICECODE-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-ICECODE-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # ICECODE-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-ICECODE-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from icecode_cli.hermes_cli.auth import has_usable_secret
