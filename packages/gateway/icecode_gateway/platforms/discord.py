...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.icecode/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("ICECODE_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("ICECODE_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields ICECODE manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your ICECODE session")
        ...
        @tree.command(name="status", description="Show ICECODE session status")
        ...
        @tree.command(name="stop", description="Stop the running ICECODE agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.icecode/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update ICECODE Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the ICECODE gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a ICECODE session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to ICECODE in the thread",
        ...
        try:
            from icecode_cli.hermes_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from icecode_cli.hermes_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a ICECODE skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from icecode_cli.hermes_cli.commands import discord_skill_commands_by_category
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by ICECODE: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "ICECODE"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by ICECODE: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "ICECODE session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 ICECODE handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ ICECODE needs your input",
                description=body,
                color=discord.Color.orange(),
            )
