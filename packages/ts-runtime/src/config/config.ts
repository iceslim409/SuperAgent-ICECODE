import * as Log from "@icecode/core/util/log"
import path from "path"
import { pathToFileURL } from "url"
import os from "os"
import z from "zod"
import { mergeDeep } from "remeda"
import { Global } from "@icecode/core/global"
import fsNode from "fs/promises"
import { NamedError } from "@icecode/core/util/error"
import { Flag } from "@icecode/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { type InstanceContext } from "../project/instance"
import { InstallationLocal, InstallationVersion } from "@icecode/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "./console-state"
import { AppFileSystem } from "@icecode/core/filesystem"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { EffectFlock } from "@icecode/core/util/effect-flock"
import { containsPath } from "../project/instance-context"
import { zod } from "@icecode/core/effect-zod"
import { NonNegativeInt, PositiveInt, withStatics, type DeepMutable } from "@icecode/core/schema"
...
export const Info = Schema.Struct({
  ...
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://icecode.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://icecode.ai/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@icecode/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@icecode/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://icecode.ai/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://icecode.ai/config.json",')
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://icecode.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://icecode.ai/config.json"
